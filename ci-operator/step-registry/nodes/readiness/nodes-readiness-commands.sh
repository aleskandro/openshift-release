...
EDGE_ZONES_COUNT=$(yq -r '.compute[] | select(.name == "edge") | .platform.aws.zones | length // 0' "${SHARED_DIR}/install-config.yaml")
expected_nodes=$(( $(yq-v4 -r '.controlPlane.replicas // '"${CONTROL_PLANE_REPLICAS:-3}" "${SHARED_DIR}/install-config.yaml") +
  ...
  $(yq-v4 -r '.compute[] | select(.name == "edge") | .replicas // '"${EDGE_WORKER_NUMBER:-${EDGE_ZONES_COUNT})}" "${SHARED_DIR}/install-config.yaml")
))
